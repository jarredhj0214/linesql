...
Oracle extensions currently include hierarchical queries, `DATE`/`TIMESTAMP` typed literals, flashback reads, DBLink table references, sequence pseudocolumns, TABLE(function), CROSS/OUTER APPLY table functions, JSON_TABLE/XMLTABLE relation sources, `ROLLUP`/`CUBE`/`GROUPING SETS`, `GROUPING_ID`, `DECODE`/`NVL2` expression dependencies, analytic window frames, `KEEP (DENSE_RANK ... ORDER BY ...)`, ordered aggregates with `WITHIN GROUP`, `LISTAGG ON OVERFLOW`, and `PERCENTILE_CONT` analytic forms, `SELECT INTO` and `BULK COLLECT INTO` variable targets, MATCH_RECOGNIZE relation usage, MODEL clause usage, materialized-view lifecycle DDL, PIVOT/UNPIVOT, SAMPLE, partition table extensions, column/table maintenance DDL, transaction controls, `FOR UPDATE NOWAIT/WAIT/SKIP LOCKED`, `LOCK TABLE ... IN ... MODE`, table object `GRANT`/`REVOKE`, package DDL, and anonymous PL/SQL block envelopes.
