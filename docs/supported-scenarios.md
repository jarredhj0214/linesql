...
| Schema/type/session/routine/trigger lifecycle and script control | `use db`, `create schema ...`, `drop schema ...`, `alter schema archive transfer object::dbo.t`, `create type ... from ...`, `create type ... as table (...)`, `drop type if exists ...`, `create or alter procedure ...`, `create or alter trigger ... on table`, `drop trigger if exists ...`, `set nocount on`, `set identity_insert dbo.t on`, `exec dbo.proc ...`, `begin/commit/rollback/save transaction`, `if ...`, `begin try ... end try begin catch ... end catch`, `print`, `raiserror`, `return` | `use_database`, `create_schema`, `drop_schema`, `alter_schema_transfer_table`, `alter_schema_transfer_object`, `create_type_alias`, `create_type_table`, `drop_type_if_exists`, `create_or_alter_procedure`, `drop_procedure`, `create_or_alter_trigger`, `drop_trigger_if_exists`, `set_nocount`, `set_identity_insert`, `execute_procedure`, `begin_transaction`, `commit_transaction`, `rollback_transaction`, `save_transaction`, `if_drop_table_control`, `try_catch_transaction_control`, `print_message_control`, `raiserror_control`, `return_status_control` |
