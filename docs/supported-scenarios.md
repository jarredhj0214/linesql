...
| Session, system, transaction, routine, and sequence lifecycle control | `alter session set current_schema = ...`, `alter system set/reset ... scope = ...`, `commit`, `rollback`, `savepoint sp`, `set transaction read only/read write/isolation level ...`, `create procedure ...`, `create function ...`, `create package ...`, `create package body ...`, `drop procedure ...`, `create/alter/drop sequence ...`, anonymous `begin/end` blocks | `alter_session_current_schema`, `alter_system_set_scope`, `alter_system_reset_scope`, `commit_statement`, `rollback_statement`, `savepoint_statement`, `set_transaction_read_only`, `set_transaction_read_write`, `set_transaction_isolation`, `create_procedure`, `create_function`, `create_package`, `create_package_body`, `drop_procedure`, `create_sequence`, `alter_sequence`, `drop_sequence`, `anonymous_begin_block`, `anonymous_declare_block` |
