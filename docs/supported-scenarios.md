...
| Schema, sequence, session, and transaction control | `create schema ...`, `drop schema ...`, `create/alter/drop sequence ...`, `set search_path to ...`, `begin/start/commit/rollback transaction`, `savepoint`, `rollback to savepoint`, `release savepoint` | `create_schema`, `drop_schema`, `create_sequence`, `alter_sequence`, `drop_sequence`, `set_search_path`, `begin_transaction`, `start_transaction_isolation`, `commit_transaction`, `rollback_transaction`, `savepoint_transaction`, `rollback_to_savepoint`, `release_savepoint` |
...
| Schema/database/type/session/routine/trigger lifecycle and script control | `use db`, `create schema ...`, `drop schema ...`, `alter schema archive transfer object::dbo.t`, `create/alter/drop database ...`, `create type ... from ...`, `create type ... as table (...)`, `drop type if exists ...`, `create or alter procedure/function ...`, `alter function ...`, `drop procedure/function if exists ...`, `create or alter trigger ... on table`, `drop trigger if exists ...`, `set nocount on`, `set identity_insert dbo.t on`, `exec dbo.proc ...`, `begin/commit/rollback/save transaction`, `if ...`, `begin try ... end try begin catch ... end catch`, `print`, `raiserror`, `return` | `use_database`, `create_schema`, `drop_schema`, `alter_schema_transfer_table`, `alter_schema_transfer_object`, `create_database_control`, `alter_database_control`, `drop_database_control`, `create_type_alias`, `create_type_table`, `drop_type_if_exists`, `create_or_alter_procedure`, `create_or_alter_function`, `alter_function`, `drop_procedure`, `drop_function_if_exists`, `create_or_alter_trigger`, `drop_trigger_if_exists`, `set_nocount`, `set_identity_insert`, `execute_procedure`, `begin_transaction`, `commit_transaction`, `rollback_transaction`, `save_transaction`, `if_drop_table_control`, `try_catch_transaction_control`, `print_message_control`, `raiserror_control`, `return_status_control` |
...
| Session, system, transaction, routine, type, and sequence lifecycle control | `alter session set current_schema = ...`, `alter system set/reset ... scope = ...`, `commit`, `rollback`, `savepoint sp`, `set transaction read only/read write/isolation level ...`, `create procedure ...`, `create function ...`, `create package ...`, `create package body ...`, `alter procedure/function/package ... compile`, `drop procedure ...`, `create/alter/drop sequence ...`, `create/drop type ...`, anonymous `begin/end` blocks, `exception` handlers, `execute immediate`, cursor `for` loops | `alter_session_current_schema`, `alter_system_set_scope`, `alter_system_reset_scope`, `commit_statement`, `rollback_statement`, `savepoint_statement`, `set_transaction_read_only`, `set_transaction_read_write`, `set_transaction_isolation`, `create_procedure`, `create_function`, `create_package`, `create_package_body`, `alter_procedure_compile`, `alter_function_compile`, `alter_package_compile_body`, `drop_procedure`, `create_sequence`, `alter_sequence`, `drop_sequence`, `create_type_object`, `create_type_table`, `drop_type_force`, `anonymous_begin_block`, `anonymous_declare_block`, `anonymous_exception_block`, `anonymous_execute_immediate_block`, `anonymous_cursor_loop_block` |
...
| MySQL control, account, admin, routine, trigger, event, and dynamic SQL statements | `unlock tables`, `set session ...`, `set names ... collate ...`, `set character set ...`, `start transaction`, `set [global/session] transaction isolation level ...`, `commit`, `rollback`, `savepoint`, `rollback to savepoint`, `release savepoint`, `xa start/end/prepare/commit/rollback/recover`, `do sleep(1)`, `call p()`, `prepare`, `execute`, `deallocate prepare`, `create procedure`, `drop procedure`, `alter procedure`, `create trigger`, `drop trigger`, `create event`, `alter event`, `drop event`, `create/alter/drop/rename user`, `set password for ...`, `show create user`, `create/drop/set role`, `grant`, `revoke`, `flush privileges/tables/binary logs/hosts`, `kill`, `reset master/binary logs/persist`, `lock/unlock instance`, `alter instance ...`, `restart`, `clone local`, `clone instance from donor`, `change replication source`, `change master`, `start/stop/reset replica` | `unlock_tables`, `set_session_statement`, `set_names_collate`, `set_character_set`, `start_transaction`, `set_transaction_read_only`, `set_transaction_isolation_repeatable_read`, `set_transaction_isolation_read_committed`, `commit_statement`, `rollback_statement`, `savepoint_statement`, `rollback_to_savepoint`, `release_savepoint`, `xa_start`, `xa_end_suspend`, `xa_prepare`, `xa_commit_one_phase`, `xa_rollback`, `xa_recover`, `do_statement`, `call_statement`, `prepare_statement`, `execute_statement`, `deallocate_prepare`, `create_procedure`, `drop_procedure`, `alter_procedure`, `create_trigger`, `drop_trigger`, `create_event`, `create_event_insert_select`, `alter_event`, `drop_event`, `create_user`, `create_user_account_options`, `alter_user_account_unlock`, `drop_user_if_exists`, `rename_user`, `set_password_for_user`, `show_create_user`, `create_role`, `drop_role`, `set_role`, `grant_privileges`, `revoke_privileges`, `flush_privileges`, `flush_tables_with_read_lock`, `flush_named_tables`, `flush_binary_logs`, `flush_hosts`, `flush_no_write_to_binlog_status`, `kill_query`, `reset_master`, `reset_binary_logs_and_gtids`, `reset_persist`, `reset_persist_if_exists`, `lock_instance_for_backup`, `unlock_instance`, `alter_instance_rotate_key`, `alter_instance_reload_tls`, `restart_statement`, `clone_local_data_directory`, `clone_instance_from_donor`, `change_replication_source`, `change_master_to`, `start_replica`, `stop_slave`, `reset_replica_all`, `reset_replica_channel` |
