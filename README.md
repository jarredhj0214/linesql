...
| PostgreSQL | Active parser | Yes | SELECT, DML, CTAS, VIEW, `ON CONFLICT`, `RETURNING`, `MERGE`, partition/inheritance DDL, schema/routine/extension DDL, privilege statements, table maintenance/locking, routine and notification control, JSON/array expressions | Direct mappings, common expressions, `DISTINCT ON`, aggregate `FILTER`, ordered-set aggregates, window frames | Predicate, merge, window, and clause usages |
