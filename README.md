...
| Hive | Active parser | Yes | Common SELECT, DML, directory export, table export/import, DDL, database lifecycle, repair/statistics statements, Hive query organization | Direct mappings, directory export columns, and common expressions | Predicate, grouping, ordering, window, sort/distribute/cluster usages |
...
| SQL Server | Active parser | Yes | Common SELECT, temporal-table reads, DML, table variables, table-valued/external query functions, DML OUTPUT capture, PIVOT/UNPIVOT relations, transaction control, backup/restore utilities, DBCC metadata/control handling, privilege DDL, synonym object references, routine lifecycle, trigger lifecycle, and index option coverage | Direct mappings and common expressions | Common predicate and clause usages |
| PostgreSQL | Active parser | Yes | SELECT, DML, CTAS, VIEW, `ON CONFLICT`, `RETURNING`, `MERGE`, generated columns, table constraints, partition/inheritance DDL, schema/routine/extension DDL, privilege statements, table maintenance/locking, routine and notification control, JSON/array expressions | Direct mappings, generated column expressions, common expressions, `DISTINCT ON`, aggregate `FILTER`, ordered-set aggregates, window frames | Predicate, merge, window, table-model, and clause usages |
