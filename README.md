...
LineSQL is usable today for Spark and MySQL lineage experiments. Hive, Flink, and StarRocks modules are already registered as scaffolds. Oracle and SQL Server are planned.

LineSQL 目前已经可以用于 Spark 和 MySQL 血缘解析实验。Hive、Flink、StarRocks 的模块架子已经接入，Oracle 和 SQL Server 在规划中。
...
| Hive | Scaffold / 架子已接入 | Hive anchors | Not implemented yet |
...
- `linesql-dialect-hive`: Hive dialect parser scaffold.
